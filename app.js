// === Machine Learning - Core Application ===
...
const App = {
    ...
    async ensureChapterLoaded(chId) {
        ...
        try { await this.loadScript(base + '-viz.js'); } catch (e) {}
        this._loadedChapters[chId] = true;
        this.chapters = window.CHAPTERS.sort((a, b) => a.number - b.number);
    },

    loadProgress() {
        try { this.progress = JSON.parse(localStorage.getItem('machine-learning-progress') || '{}'); }
        catch (e) { this.progress = {}; }
    },
    saveProgress() {
        try { localStorage.setItem('machine-learning-progress', JSON.stringify(this.progress)); } catch (e) {}
    },
    ...
    async loadChapter(chId) {
        ...
        try { await this.ensureChapterLoaded(chId); } catch (e) {
        ...
        if (!ch) { document.getElementById('section-content').innerHTML = '<p style="color:var(--accent-red);text-align:center;padding:60px 0;">Chapter data not found.</p>'; this._loading = false; return; }
        this.currentChapter = ch;
        this.currentSectionIndex = 0;
        window.location.hash = chId;
        document.querySelectorAll('.chapter-item').forEach((item, i) => { item.classList.toggle('active', this.manifest[i]?.id === chId); });
    ...
    renderSectionTabs() {
        const c = document.getElementById('section-tabs'); c.innerHTML = '';
        if (!this.currentChapter) return;
        this.currentChapter.sections.forEach((sec, i) => {
            const tab = document.createElement('button'); tab.className = 'section-tab';
    ...
    loadSection(idx) {
        ...
        if (this._activeVizEngines) { this._activeVizEngines.forEach(v => v.stopAnimation()); }
        ...
        el.innerHTML = this.escapeMathHtml(sec.content); el.className = 'fade-in';
        ...
        if (extraViz && extraViz.length > 0) {
            for (const viz of extraViz) { const ph = document.createElement('div'); ph.className = 'viz-placeholder'; ph.setAttribute('data-viz', viz.id); el.appendChild(ph); }
    ...
    renderMath(el) {
        if (typeof renderMathInElement === 'function') {
            renderMathInElement(el, { delimiters: [{ left: '$$', right: '$$', display: true }, { left: '\\[', right: '\\]', display: true }, { left: '\\(', right: '\\)', display: false }], throwOnError: false, trust: true });
        }
    },
    ...
    initVisualizations(vizList) {
        document.querySelectorAll('.viz-placeholder').forEach(ph => {
            ...
            if (!viz) return;
            const container = document.createElement('div'); container.className = 'viz-container';
            const header = document.createElement('div'); header.className = 'viz-header';
            const title = document.createElement('span'); title.className = 'viz-title';
            ...
            const controls = document.createElement('div'); controls.className = 'viz-controls'; header.appendChild(controls);
            const body = document.createElement('div'); body.className = 'viz-body';
            container.appendChild(header); container.appendChild(body);
            if (viz.description) { const desc = document.createElement('div'); desc.className = 'viz-description'; desc.innerHTML = viz.description; if (window.renderMathInElement) renderMathInElement(desc, {delimiters:[{left:'\\(',right:'\\)',display:false},{left:'\\[',right:'\\]',display:true}],throwOnError:false}); container.appendChild(desc); }
            ph.replaceWith(container);
            try { if (typeof viz.setup === 'function') { const engine = viz.setup(body, controls); if (engine && engine.stopAnimation) this._activeVizEngines.push(engine); } }
            catch (e) { console.error('Viz error:', vizId, e); body.innerHTML = '<p style="color:#f85149;">Visualization failed to load: ' + e.message + '</p>'; }
        });
    },
    ...
    renderExercises(exercises) {
        ...
        const freshList = list.cloneNode(false); list.parentNode.replaceChild(freshList, list);
        exercises.forEach((ex, i) => {
            ...
            item.appendChild(q);
            const toggle = document.createElement('div'); toggle.className = 'exercise-toggle';
            if (ex.hint) { const hBtn = document.createElement('button'); hBtn.textContent = 'Show Hint'; hBtn.setAttribute('data-action', 'hint'); hBtn.setAttribute('data-idx', i); toggle.appendChild(hBtn); }
            const sBtn = document.createElement('button'); sBtn.textContent = 'Show Solution'; sBtn.setAttribute('data-action', 'solution'); sBtn.setAttribute('data-idx', i); toggle.appendChild(sBtn);
            item.appendChild(toggle);
            if (ex.hint) { const hDiv = document.createElement('div'); hDiv.className = 'exercise-hint'; hDiv.id = 'exhint-' + i; hDiv.innerHTML = this.escapeMathHtml(ex.hint); item.appendChild(hDiv); }
            const solDiv = document.createElement('div'); solDiv.className = 'exercise-solution'; solDiv.id = 'exsol-' + i; solDiv.innerHTML = this.escapeMathHtml(ex.solution || ''); item.appendChild(solDiv);
            freshList.appendChild(item);
        });
        freshList.addEventListener('click', (e) => {
            const btn = e.target.closest('button[data-action]'); if (!btn) return;
            const action = btn.getAttribute('data-action'); const idx = btn.getAttribute('data-idx');
            const prefix = action === 'hint' ? 'exhint-' : 'exsol-';
            const el = document.getElementById(prefix + idx);
            if (el) { el.classList.toggle('show'); const label = action.charAt(0).toUpperCase() + action.slice(1); btn.textContent = el.classList.contains('show') ? 'Hide ' + label : 'Show ' + label; if (el.classList.contains('show')) this.renderMath(el); }
        });
        this.renderMath(freshList);
    },

    updateNavButtons() {
        const prev = document.getElementById('prev-btn'); const next = document.getElementById('next-btn');
    ...
    prevSection() { if (!this.currentChapter || this.currentSectionIndex <= 0) return; this.loadSection(this.currentSectionIndex - 1); },
    ...
    bindEvents() {
        ...
        document.addEventListener('keydown', (e) => { if (e.target.tagName === 'INPUT') return; if (e.key === 'ArrowRight' || e.key === 'n') this.nextSection(); if (e.key === 'ArrowLeft' || e.key === 'p') this.prevSection(); });
    },
    filterChapters(q) { q = q.toLowerCase(); document.querySelectorAll('.chapter-item').forEach((item, i) => { if (i < this.manifest.length) { const m = this.manifest[i]; item.style.display = (!q || m.title.toLowerCase().includes(q)) ? '' : 'none'; } }); },
    checkHash() { const h = window.location.hash.slice(1); if (h) this.loadChapter(h); }
};
